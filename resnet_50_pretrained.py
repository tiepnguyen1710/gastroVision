...
def get_args():
    ...
    parser.add_argument('-e','--epochs', metavar='E', type=int, default=15, nargs='?', help='Number of epochs', dest='max_epochs')
    parser.add_argument('-b','--batch-size', metavar='B', type=int, default=16, nargs='?', help='Batch size', dest='batch_size')
    parser.add_argument('-l','--learning-rate', metavar='LR', type=float, default=0.001, nargs='?', help='Learning rate', dest='lr')
...
train_root_dir=f"./kvasir/train"
val_root_dir=f"./kvasir/val"
test_root_dir=f"./kvasir/test"
model_path=r'./checkpoints3/'  # set path to the folder that will store model's checkpoints

n_classes=8  # number of classes used for training
...
filename='results_e'+str(max_epochs)+'_'+'b'+str(batch_size)+'_'+'lr'+str(lr)+'_'+'resnet50_pretrained'   #filename used for saving epoch-wise training details and test results 
...
class train:
    ...
    def train_net(self):
        ...
        model = torchvision.models.resnet50(weights='IMAGENET1K_V1').to(device)   # make weights=True if you want to download pre-trained weights 	        
