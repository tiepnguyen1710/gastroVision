...
def get_class_weights(dataset):
    # Tính toán class weights dựa trên số lượng ảnh trong mỗi class
    class_counts = torch.zeros(n_classes)
    for _, label in dataset:
        class_counts[label] += 1
    
    # Tính weights ngược tỷ lệ với số lượng ảnh
...
class train:
    def __init__(self):
        ...
        for i, (name, count) in enumerate(zip(class_names, class_counts)):
            logging.info(f"Class {name}: {int(count)} samples")
            
        # Calculate and print class weights
        class_weights = get_class_weights(training_dataset)
        logging.info("\nClass weights:")
        ...
        logging.info(f'Validation set: {len(validation_dataset)} images')
        logging.info(f'Test set: {len(test_dataset)} images')
        
    def train_net(self):
        
        model = torchvision.models.densenet121(weights='IMAGENET1K_V1').to(device)
